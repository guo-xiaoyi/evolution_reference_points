# -*- coding: utf-8 -*-
"""
High-Performance Discrete Lottery Optimizer with File Output

This script produces high-stake four-outcome lotteries.
...
def fast_create_lottery_structure(params):
    """JIT compiled lottery structure creation"""
...
def fast_check_basic_constraints(params):
    """JIT compiled basic constraint checking"""
    b11, b12, c21, c22, c31, c32, c33, c34, p1, p2, p3 = params
    ...
    Z, p = fast_create_lottery_structure(params)
    z1, z2, z3, z4 = Z
...
class OptimizedLotteryOptimizer:
    """High-performance complete lottery optimizer"""
    ...
    def _preallocate_memory(self):
        ...
        self.temp_outcomes = np.empty(4, dtype=np.float64)
        self.temp_probabilities = np.empty(4, dtype=np.float64)
        self.temp_params = np.empty(11, dtype=np.float64)

    def generate_batch_params(self, batch_size: int) -> np.ndarray:
        """Generate parameters in batches"""
    ...
    def fast_prefilter(self, params_batch: np.ndarray) -> np.ndarray:
        ...
        return params_batch[valid_indices] if valid_indices else np.empty((0, 11))
    ...
    def check_full_constraints(self, params):
        """Full constraint checking - consistent with original version"""
        try:
            ...
            # Constraint 2: Monotonic interval for L
            ...
            # Constraint 3: Monotonic interval for L1
            Z_L1 = np.array([z1, z2])
            ...
            # Constraint 4: Monotonic interval for L2
            Z_L2 = np.array([z3, z4])
            ...
            if IL2_lower is None or IL2_upper is None or IL2_lower >= IL2_upper:
                ...
                values_to_check_L2 = [b12, b12 + c22, E_L2, expected_value]
    ...
    def save_solutions_to_file(self, solutions: List[CompleteSolution], filename: str = None) -> str:
        """Save solutions to text file"""
        if filename is None:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = os.path.join(self.config.output_dir, f"lottery_solutions_four_{timestamp}.txt")

        try:
            with open(filename, 'w', encoding='utf-8') as f:
                # Write file header
                f.write("=" * 80 + "\n")
                f.write("High-Performance Discrete Lottery Optimizer - Results Report\n")
                f.write("=" * 80 + "\n")
                ...
                for i, solution in enumerate(solutions):
                    ...
                    # Sub-lottery intervals with corrected expected value calculation
                    Z_L1 = np.array([z1, z2])
                    p_L1 = np.array([solution.p2, 1 - solution.p2])
                    E_L1 = np.sum(Z_L1 * p_L1)  # Corrected: scalar expected value
                    ...
                    E_L2 = np.sum(Z_L2 * p_L2)  # Corrected: scalar expected value
                ...
                for i, sol in enumerate(solutions):
                    ...
                    summary_data.append({
                        ...
                        'c31': sol.c31, 'c32': sol.c32,
                        'c33': sol.c33, 'c34': sol.c34,
                ...
                if len(solutions) == 0:
                    f.write("Suggestions for no solutions found:\n")
                    f.write("  - Increase num_attempts (current: {:,})\n".format(self.config.num_attempts))
                    f.write("  - Relax violation_threshold (current: {:.1f})\n".format(self.config.violation_threshold))
                    f.write("  - Expand parameter search ranges\n")
                    f.write("  - Adjust prospect theory parameters\n")
                elif len(solutions) < 10:
                    f.write("Suggestions for few solutions:\n")
                    f.write("  - Increase search attempts\n")
                    f.write("  - Moderately relax violation threshold\n")
                    f.write("  - Use more probability choices\n")
                else:
                    f.write("Search performed well!\n")
                    f.write("  - Found sufficient solutions\n")
                    f.write("  - Can further analyze solution quality\n")
                    f.write("  - Consider stricter constraints\n")
    ...
    def save_progress(self, solutions: List[CompleteSolution], filename: str = None):
        ...
        if filename is None:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = os.path.join(self.config.output_dir, f"lottery_progress_{timestamp}.pkl")
...
def main():
    """Main function"""
    print("High-Performance Discrete Lottery Optimizer")
    print("=" * 50)
    ...
    if solutions:
        ...
        print(f"\nTop {min(5, len(solutions))} best solutions:")
        for i, sol in enumerate(solutions[:5]):
            print(f"  {i+1}. Violation: {sol.violation:.6f}, Lottery: {sol.lottery_values}, Probs: {[f'{p:.3f}' for p in sol.probabilities]}")
