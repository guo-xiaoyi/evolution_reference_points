...
def load_lotteries():
    return _load_lottery_file(LOTTERIES_PATH)


def load_calibration_lotteries():
    return _load_lottery_file(CALIBRATION_LOTTERIES_PATH)


def load_test_lotteries():
    return _load_lottery_file(PRACTICE_LOTTERY_PATH)
...
def lottery_period_keys(lottery, include_start=False):
    """Return sorted integer period keys from a lottery definition."""
    ...
    for key in periods.keys():
        try:
            ...
            continue
        if include_start or key_int > 0:
            keys.append(key_int)
    return sorted(set(keys))


def lottery_last_period(lottery):
    """Return the last realized payoff period for a lottery."""
    keys = lottery_period_keys(lottery, include_start=False)
...
def get_participant_lottery_order(participant, store_key='lottery_order', lotteries=None):
    """Return (and store) a participant-specific lottery order."""
    order = participant.vars.get(store_key)
    if not order:
        Constants = _constants()
        source_lotteries = lotteries or Constants.lotteries
        order = build_lottery_order(source_lotteries)
        participant.vars[store_key] = order
    return order
...
def compute_final_payoff(store):
    ...
    lottery_id = store.get('lottery_id') or Constants.default_lottery
    lottery = Constants.lotteries.get(lottery_id, Constants.lotteries[Constants.default_lottery])
    final_period = lottery_last_period(lottery)
    realized = store.get('realized_nodes', {})
    final_node = realized.get(final_period) if final_period is not None else None
    if not final_node:
        return None
...
def build_play_context(player, choice_lottery, display_lottery=None, base_offset=0):
    ...
    is_calibration_round = bool(
        round_number is not None and 1 <= round_number <= calibration_rounds
    )
    is_main_evaluation_round = bool(
        round_number is not None and round_number in Constants.paying_round_numbers
    )
    return {
        ...
        'session1_rounds': session1_rounds,
        'calibration_rounds': calibration_rounds,
        'evaluation_rounds': evaluation_rounds,
        'is_calibration_round': is_calibration_round,
        'is_main_evaluation_round': is_main_evaluation_round,
    }
